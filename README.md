...
- **`genes`** — `gene_id` (CCNA_XXXXX NA1000 locus tag), `cc_tag`
  (CC_XXXX CB15 legacy tag), `gene_name`, `ncbi_protein_id`,
  `gene_biotype`, `description`
- **`experiments`** — metadata for each dataset: `experiment_id`,
  `display_label`, `experiment_class` (`de_comparison` \| `timecourse`),
  `data_type`, `strain`, `lab_group`, `doi`
- **`experiment_conditions`** — ordered condition labels with optional
  numeric values and units; FK → `experiments`
- **`de_results`** — `log2fc` and `padj` per gene per experiment; FK →
  `genes` + `experiments`
- **`timecourse_expression`** — `expression_value` per gene per
  condition per experiment; FK → `genes`, `experiments`, and
